...
- **perf: keyframe-map build no longer dominates `open()`** — the per-chunk `keyframe_by_offset` map (one insert per indexed packet chunk, built from `idx1` + every `ix##` entry) is now pre-sized to the exact entry count and keyed through a cheap Fibonacci-multiplicative `u64` hasher instead of the DoS-resistant default (the keys are self-computed file offsets, not attacker-chosen input). Profiling the round-415 bench suite showed hasher + rehash-and-move churn as the single largest `open()` cost. `open/avi10` 87.9 µs → 40.9 µs (−53%), `open/opendml` 118.0 µs → 66.8 µs (−43%) on the 3000-packet fixture; demuxed packets, flags, and every accessor unchanged (round 415)
- **perf: `next_packet` chunk walk — no double-touch body reads, one position query per chunk** — `read_body_bounded` read each chunk body into a `resize`-zeroed buffer, memset-touching every payload byte twice; it now reads straight into uninitialised spare capacity through a `Take`-bounded `read_to_end` (the round-394 hostile-`cb` allocation bound is preserved — growth is driven by bytes actually present, initial capacity still capped at 64 KiB — and truncation surfaces the same `UnexpectedEof` kind). `next_packet` also folded its two per-chunk `stream_position()` calls (segment-end check + keyframe-map key) into one. `walk/avi10` −2.4%, `walk/opendml` 193.4 µs → 179.7 µs (−7.2%) on the 3000-packet fixture (round 415)
- **perf: seek tables built once, not per seek call** — `seek_to` re-scanned the FULL `idx1` table (every entry of every stream) per seek, and the OpenDML std-index path re-allocated and re-derived the requested stream's whole `(offset, pts, keyframe)` table per seek plus a second all-streams walk for the counter reset. A lazily-built `SeekCache` (first `seek_to` call; the source tables are immutable after `open()`) now holds per-stream keyframe and entry tables, replicating the historical walks entry-for-entry — same iteration order, same saturating pts arithmetic, keep-first-of-equal-pts landing rule — with binary search when the derived table is monotonic (every writer this crate has seen, including its own muxer) and a verbatim linear fallback for hostile/unordered indexes. `seek/idx1_32_seeks` 56.7 µs → 0.94 µs (−98.3%, 1.77 µs → 29 ns per seek), `seek/stdindex_32_seeks` 185.0 µs → 1.01 µs (−99.5%, 5.78 µs → 32 ns per seek); `open()` pays nothing (cache is lazy) and landed pts / post-seek packet streams are identical (round 415)
- **perf: muxer write path — one position query per packet, one write per chunk header** — `write_packet` issued up to three `stream_position` calls per packet (OpenDML segment-roll projection, index-entry offset, AVI 1.0 2-GiB ceiling check); the position is now queried at most once and cached (invalidated whenever an intervening write — segment roll, `LIST rec ` open/close, alignment JUNK — moves it), with the 2-GiB check derived arithmetically from the recorded chunk offset. `write_chunk_header` also emits the FourCC + size as a single 8-byte write instead of two 4-byte writes (halves the small-write count on every writer backend, benefiting all header/index emit paths too). `mux/avi10` 166.2 µs → 156.0 µs (−5.8%), `mux/opendml` 175.0 µs → 164.4 µs (−6.1%); output bytes are identical — gated by the round-415 golden-hash pins (round 415)
- **perf: `open()` idx1 ingestion — one side-band scan instead of two, side-band body walks skipped when empty, per-stream tick divisor hoisted** — the `pc` and `tx` side-band counts are now collected in a single pass over the raw idx1 (`scan_idx1_sideband_counts`, previously two full `scan_idx1_for_suffix` passes); the eager `xxpc`/`xxtx` body walks and the palette-position derivation (three more full passes, two with offset probes + seeks) are skipped entirely when the counts say no qualifying entry exists — output-identical, since with no entries they fill nothing; and the idx1 pts-assignment pass applies a per-stream precomputed block-align divisor (`time_delta_divisor`) instead of re-deriving it per entry through `packet_time_delta`. Same divisor hoist applied to the round-415 seek-cache build. `open/avi10` 40.9 µs → 30.3 µs (further −26%; −65.8% vs the round-415 baseline), `open/opendml` 66.8 µs → 63.8 µs. Also fixed the CI clippy toolchain-drift `for_kv_map` lint on the pre-existing 2-field metadata loop (round 415)
- **perf: `scan_ix_in_movi` tracks its position arithmetically** — the OpenDML `ix##` scan issued two `stream_position` queries per walked `movi` chunk (segment-bound check + chunk-header offset); every loop operation advances the stream by a known amount, so the position is now tracked in a local (single resync only on a truncated body read). Neutral on the in-memory Criterion fixture (a `Cursor` position query is a cheap virtual call) but removes two `lseek` syscalls per chunk per segment on file-backed input — the walked chunk count is every chunk in every `movi` segment (round 415)
- **golden-hash pins for the muxer write path** (`tests/round415_golden_mux_hash.rs`): FNV-1a-64 pins over the finished file bytes of a fixed 64-frame-pair two-stream fixture (incl. one odd-length payload to pin pad-byte + `idx1` offset behaviour) through both `AviKind::Avi10` and `AviKind::OpenDml(64 KiB)` (segment roll + `ix##` flush + super-index patch paths). Regression gate for the round-415 optimization passes: any accidental byte drift in header suite, chunk layout, index build, or finalize patching fails the pin (round 415)
- **citation hygiene**: replaced the remaining invented "spec/06 §6.1" references (no §6.1 exists in `opendml-avi-2.0.pdf`; one carried a fabricated quote and a stale "per-stream `ix##` chunks are intentionally omitted" claim contradicting the writer's actual output) with the real section anchors — §2.0 "Extension for File Size > 1 GB" for the `RIFF AVIX` multi-chunk form, §3.0 "AVI Super Index Chunk" / "Index Locations in RIFF File" for the index layouts, AVI 1.0 §"AVI RIFF Form" for `LIST rec ` grouping, and the correctly-numbered §5.0 references now name the PDF explicitly. Also fixed the round-394 fuzz-harness fixture builder to use per-call-unique temp filenames — the three mutation suites built the same fixture set concurrently and raced create/read/remove on shared paths (the CI flake on the previous push) (round 394)
- **black-box ffmpeg/ffprobe validation of the round-394 writer shapes** (`tests/round394_ffmpeg_blackbox.rs`, 4 suites, skipped gracefully when the binaries are absent; binaries discovered across `/opt/homebrew/bin` + `/usr/local/bin` + `/usr/bin` — the older `avi_ffmpeg.rs` hardcodes `/usr/bin/ffmpeg` and silently skipped on Homebrew machines): ffprobe `-count_packets` confirms every packet of a multi-segment OpenDML file (per-stream `indx`, spec-correct super-index targets) and of a compact in-`strl`-standard-index file is reachable by an independent reader; ffprobe classifies our muxed `txts` stream as `subtitle` and counts its packets; and a full demux→remux round-trip over an ffmpeg-encoded mjpeg+pcm source revalidates through ffprobe with matching per-stream packet counts (round 394)
- **index-walker fuzz hardening + bounded chunk-body allocation**: `read_body_bounded` committed a `vec![0; cb]` allocation for the DECLARED chunk size before reading — a corrupt / hostile `cb` near `0xFFFFFFFF` requested ~4 GiB up-front; the buffer now grows in 64 KiB steps so memory stays bounded by actual stream content. New deterministic in-tree mutation harness `tests/round394_fuzz_index_walkers.rs` (3 suites, reproducible xorshift64* seed): ~240 random byte-flip mutants + ~150 truncation points + targeted `idx1` / `indx` / `ix##` header+body corruption (hostile counts, all-ones sizes, zeroed strides) over three writer shapes (legacy AVI 1.0 with side-band chunks, multi-segment OpenDML, compact in-`strl` standard index); every mutant is opened strict + lenient and run through the full index-accessor battery, a bounded packet walk, and both seek paths — no panics, no unbounded allocation (round 394)
- **first-class `txts` text streams — demux packets + mux `MediaType::Subtitle`** (AVI 1.0 §"AVISTREAMHEADER" `fccType` row: *"txts (text stream)"*; `##tx` chunk family per `mmsystem.h`): a declared `txts` stream now demuxes as `MediaType::Subtitle` (codec id `avi:txts`, raw `strf` bytes surfaced verbatim as extradata — the spec pins no text stream-format layout) with its `##tx` chunks delivered as packets through `next_packet`. Pre-round-394 the stream was `MediaType::Data` with a never-matching `xx` packet suffix and every `##tx` chunk was unconditionally side-banded, so a text stream's own packets were unreachable. The side-band `text_chunk_count` / `text_chunk_data` surfaces stay reserved for `##tx` chunks riding on streams NOT declared `txts` (e.g. subtitle overlays interleaved under a video stream's slot); for declared text streams they read 0/empty. Muxer: `MediaType::Subtitle` streams are now packagable — `strh.fccType = txts`, packet suffix `tx`, `strf` = `params.extradata` verbatim (empty ⇒ zero-length `strf`), `(dwScale, dwRate)` from the stream's own time base, `dwSampleSize = 0`; text streams participate in `idx1`, `ix##`, and the per-stream `indx` like any other stream. Tests: `tests/round394_txts_streams.rs` (4: AVI-1.0 + OpenDML round-trips incl. per-packet keyframe flags and `02tx` super-index chunk id, video-slot side-band regression, empty-extradata strf) (round 394)
- **in-`strl` standard index — the OpenDML compact single-index layout — read + write** (`docs/container/riff/opendml-avi-2.0.pdf` §"Index Locations in RIFF File": the strl `indx` *"may either be an index of indexes (super index), or may be an index to the chunks directly. … If the 'indx' chunk is a standard or field index chunk (i.e., not an index of indexes) then the stream has only one index chunk and there is none in the 'movi' data."*). Demuxer: `parse_strl` now parses an `AVI_INDEX_OF_CHUNKS` `indx` as that stream's single standard (or `AVI_INDEX_2FIELD` field) index and folds it into the same per-stream std-index machinery that backs OpenDML seek, per-packet keyframe flags, `field2_offset_for_packet`, and every `std_index_*` accessor / validator — pre-round-394 such an `indx` was dropped entry-less, and one with `nEntriesInUse * 16 > body` even **failed `open()`** because the super-index 16-byte-stride truncation check ran before the `bIndexType` check (check order fixed). Muxer: `AviMuxOptions::with_strl_std_index(capacity)` reserves each stream's `indx` as the compact standard index (8-byte entries; 12-byte field-index entries with `dwOffsetField2` for `with_field2_stream` streams) and emits NO `ix##` chunks; on a `RIFF AVIX` roll or a capacity overflow it falls back transparently — `ix##` tail chunks plus the same reserved bytes re-patched as an `AVI_INDEX_OF_INDEXES` super-index (the spec's *"file can be easily grown"* migration; `patch_super_index` now rewrites the full 24-byte preamble to support the shape change). Ignored for `AviKind::Avi10` / `with_mid_movi_index` combos. Tests: `tests/round394_strl_std_index.rs` (5: roundtrip incl. delta-bit keyframes + strict std-index seek, AVIX-roll fallback, capacity-overflow fallback, 2-field field-index, stride-regression open) (round 394)
- **per-stream `indx` super-index on write** (`docs/container/riff/opendml-avi-2.0.pdf` §"Index Locations in RIFF File": *"Unlike the 'idx1' chunk, a single index is stored per stream in the AVI file. An 'indx' chunk follows the 'strf' chunk in the LIST 'strl' chunk of an AVI header."*): the OpenDML muxer now emits an `indx` super-index in **every** stream's `strl` — previously stream 0 only, leaving the other streams' per-segment `ix##` chunks unreachable from any `strl`-level index. Each stream's `indx` is back-patched in `write_trailer` with its own `ix##` chunk records (`dwChunkId` = that stream's packet FourCC, e.g. `01wb`; audio `dwDuration` in sample ticks), and the `AVI_INDEX_2FIELD` sub-type stamp is per-stream (`with_field2_stream(n)` lands on stream `n`'s `indx`, not hard-wired to stream 0). `truncated_super_index_segments()` reports the worst-case overflow across streams. Also rebuilt the `write_trailer` `strh.dwLength` / `strh.dwSuggestedBufferSize` patch to use offsets recorded at `write_header` time (`TrackState::strh_body_off`) instead of an arithmetic layout walk — the old walk didn't account for optional `strd` / `strn` chunks, so a multi-stream file with a stream name / driver blob on an earlier stream had the later streams' `strh` patches written into the wrong bytes. Tests: `tests/round394_per_stream_indx.rs` (5, incl. the strn/strd patch regression on both envelopes) (round 394)
- **spec-correct OpenDML super-index entry targets** (`docs/container/riff/opendml-avi-2.0.pdf` §"AVI Super Index Chunk"): each used `_avisuperindex_entry` carries the *"absolute file offset"* of an `ix##` AVISTDINDEX chunk in `qwOffset` (the spec marks `0` as an UNUSED entry), the *"size of index chunk at this offset"* in `dwSize`, and the *"time span in stream ticks"* in `dwDuration`. The muxer previously stamped the enclosing **RIFF segment's** offset/size — unusable by a conformant reader dereferencing the super-index, and the primary segment's `0` offset read as the unused-entry sentinel (the miscited "spec/06 §6.1" quote backing that behaviour does not exist in the spec). `write_trailer` now back-patches one entry per flushed `ix##` chunk of the indexed stream — mid-`movi` periodic flushes (`with_mid_movi_index`) each get their own entry, matching the spec's §"Index Locations in RIFF File" growth model — with `dwDuration` as that chunk's tick span (frames for video, `dwLength` units for audio). Demuxer: new `super_index_entries(stream) -> Option<Vec<SuperIndexEntryInfo>>` verbatim-triple accessor (`None` = no `indx`, `Some(vec![])` = present-but-empty), `super_index_target_violations() -> Vec<SuperIndexTargetViolation>` cross-check comparing each non-zero `qwOffset` against the scanned `ix##` chunk-header positions (informational — this demuxer scans `movi`, so stale-super-index files keep demuxing), and divergence-only `avi:indx.<n>.stale_targets = "<count>"` metadata key. `truncated_super_index_segments()` now counts overflow against the per-`ix##` entry list. Tests: `tests/round394_super_index_targets.rs` (5) + rewritten `opendml_indx_super_index_entries_point_at_ix_chunks` (round 394)
...
- OpenDML 2.0 `dmlh` (Extended AVI Header) declared body size + trailing reserved bytes read + write symmetry (`docs/container/riff/opendml-avi-2.0.pdf` §"Extended AVI Header (dmlh)"): the spec documents the `dmlh` body as exactly the 4-byte `dwTotalFrames` DWORD, but real OpenDML writers allocate a larger zero-padded body (the spec's own worked-example layout shows an 84-byte `dmlh`) reserving space after `dwTotalFrames`. Demuxer surfaces `dmlh_declared_body_size() -> Option<u32>` (the physically-declared `cb`) and `dmlh_reserved() -> Option<&[u8]>` (the verbatim, uninterpreted bytes after `dwTotalFrames` — `Some(&[])` for the spec-minimal 4-byte body, `None` when no `dmlh`), via a new `DmlhInfo` layout struct; `parse_odml_list` now tolerates a larger `dmlh` body and retains the reserved tail instead of discarding it. Muxer gains `AviMuxOptions::with_dmlh_body_size(n)` (`n` clamped up to 4) which writes `dwTotalFrames` followed by `n - 4` zero reserved bytes + a RIFF word-pad for odd sizes; builder→writer→demuxer round-trips the declared size + reserved length, and `dwTotalFrames` (incl. the `with_dmlh_total_frames` override) stays intact regardless of padding (round 377)
- top-level `CSET` (character-set) chunk read + write symmetry (generic RIFF chunk; `docs/container/riff/metadata/exiftool-riff-tags.html` "RIFF CSET Tags"): the body is a fixed 8-byte record of four little-endian 16-bit fields in RIFF CSET tag-index order — `wCodePage` / `wCountryCode` / `wLanguageCode` / `wDialect` — overriding the character set / language / country / dialect of the form's text chunks (`0,0,0,0` = use the current locale). Demuxer records each top-level `CSET` chunk's file-absolute header offset + the four typed fields (primary RIFF only) via `cset_chunks() -> &[CsetChunk]` / `cset_chunk_count() -> usize` + `avi:cset.count` metadata key (omitted when no `CSET`); `CsetChunk::to_bytes()` serialises the four fields back to the canonical 8-byte body. Muxer gains `AviMuxOptions::with_cset(cset)` + `with_cset_fields(cp, cc, lc, d)` (repeatable), emitting one `CSET` chunk per call as a sibling of `LIST hdrl` just before `movi` (8-byte body is even, no word-pad); builder→writer→demuxer round-trips the four typed fields equal (round 377)
- `AVIF_HASINDEX` flag ↔ `idx1`-presence cross-check per AVI 1.0 §"AVI Index Entries" (*"If the file contains an index, set the AVIF_HASINDEX flag in the dwFlags member of the AVIMAINHEADER structure"*): `has_index_flag_violation() -> Option<(bool, bool)>` returns `Some((flag_set, idx1_present))` when the `avih.dwFlags` `AVIF_HASINDEX` bit disagrees with whether an `idx1` chunk is physically present, `None` when they agree. `idx1` presence is the chunk being present (a present `idx1` holding only `rec ` LIST entries still counts), distinct from the seek table being non-empty. Informational, never fails `open()`, same family as `declared_vs_actual_stream_count_mismatch` / `palette_change_flag_violations`; the demuxer always uses the index it physically found regardless of the flag (round 373)
- raw `WAVEFORMATEX.nBlockAlign` accessor + VBR/CBR classification per AVI 1.0 §"AVISTREAMHEADER": `stream_block_align(stream) -> Option<u16>` surfaces the raw `nBlockAlign` for **any** audio stream (the `0` "unspecified" stamp maps to `None`) — distinct from `cbr_audio_block_alignment_violations`, which only consults CBR streams with `nBlockAlign > 1`, so this also exposes a VBR stream's advisory `nBlockAlign` (typically 1). `audio_is_vbr(stream) -> Option<bool>` classifies the stream from its `wFormatTag` (`Some(true)` VBR codec / `Some(false)` CBR codec / `None` non-audio or unconstrained tag) — the same classification behind the `open()`-time C2 sample-size invariant, letting a caller decide whether `nBlockAlign` is a hard framing constraint (CBR) or a hint (VBR) (round 373)
- top-level `DISP` display / sound-scheme chunk read + write symmetry (RIFF tag registry, `docs/container/riff/metadata/exiftool-riff-tags.html` maps `'DISP'` → `SoundSchemeTitle`): demuxer records each top-level `DISP` chunk's file-absolute header offset, declared body size, and the **raw body bytes** verbatim (primary RIFF only) via `disp_chunks() -> &[DispChunk]` / `disp_chunk_count() -> usize` + `avi:disp.count` metadata key (omitted when no `DISP`). The leading 4-byte clipboard-format code is NOT split out — that internal layout is not in the in-tree docs, so the raw bytes are surfaced and the caller decides how to interpret them. Muxer gains `AviMuxOptions::with_disp_chunk(body)` (repeatable), emitting one `DISP` chunk per call as a sibling of `LIST hdrl` just before `movi`; builder→writer→demuxer round-trips the raw body byte-equal (round 373)
- `AVISF_VIDEO_PALCHANGES` flag ↔ `xxpc`-presence cross-check per AVI 1.0 §"Stream Data ('movi' List)" (*"If a stream contains palette changes, set the AVISF_VIDEO_PALCHANGES flag ... for that stream"*): `palette_change_flag_violations() -> Vec<PaletteChangeFlagViolation>` returns one `{ stream_index, flag_set, palette_change_chunks }` per video stream whose `AVISTREAMHEADER.dwFlags` `AVISF_VIDEO_PALCHANGES` bit disagrees with whether it actually carries `xxpc` chunks — fires for both the missing-hint case (xxpc present, flag clear) and the spurious-hint case (flag set, no xxpc). Informational, never fails `open()`, same family as `declared_vs_actual_stream_count_mismatch` / `cbr_audio_block_alignment_violations`; scoped to video streams. The muxer's `write_palette_change` does not set the flag itself, so a conforming file pairs it with `with_stream_flags(stream, AVISF_VIDEO_PALCHANGES)` (round 373)
- multi-RIFF `LIST movi` segment-boundary surface per OpenDML 2.0 §"RIFF Chunk Format": `movi_segments() -> &[(u64, u64)]` returns the file-absolute `[start, end)` byte range of every `LIST movi` segment in file order (`start` points just past the 4-byte `movi` form-type FourCC, so the FourCC sits at `start - 4`; these are the same boundaries `next_packet` walks across, so a caller can map a packet's file position back to its RIFF segment), `movi_segment_count() -> usize`, and `avix_segment_count() -> u32` (number of `RIFF AVIX` extension chunks beyond the primary `RIFF AVI `, `0` for a single-RIFF file). Paired `avi:riff.avix_count` / `avi:movi.segments` metadata keys, both omitted for the single-RIFF / single-movi case so the plain AVI 1.0 file stays free of OpenDML-specific keys. The demuxer already tracked these segments internally for the AVIX continuation walker + OpenDML seek path but never exposed them publicly (round 373)
- top-level `JUNK` padding-chunk read + write symmetry per AVI 1.0 §"Other Data Chunks" (*"Data can be aligned in an AVI file by inserting 'JUNK' chunks as needed. Applications should ignore the contents of a 'JUNK' chunk."*): demuxer records each top-level `JUNK` chunk's file-absolute header offset + declared body size across the primary RIFF + every `RIFF AVIX` continuation, surfaced via `junk_chunks() -> &[JunkChunk]` / `junk_chunk_count() -> usize` / `junk_total_bytes() -> u64` accessors + `avi:junk.count` / `avi:junk.total_bytes` metadata keys (both omitted when the file carries no JUNK so absence stays observable); contents stay ignored — JUNK never enters the packet stream. Muxer gains `AviMuxOptions::with_top_level_junk(body_size)` (repeatable) which emits one `JUNK` chunk per call as a sibling of `LIST hdrl` just before the `movi` LIST, outside `movi` so it never shifts packet / `idx1` / `ix##` offsets — distinct from the per-packet alignment JUNK driven by `with_padding_granularity`. Builder→writer→demuxer round-trips byte-for-byte (round 373)
- muxer-side `vprp` signal-shape round-trip: `VprpConfig` gains `h_total_in_t` / `v_total_in_lines` / `frame_width_in_pixels` / `frame_height_in_lines` fields + `with_signal_totals(h, v)` / `with_active_frame(w, h)` / `with_vertical_refresh_rate(hz)` builders, each `0` falling back to the stream's coded dimension (the pre-round-365 default that hardcoded these four DWORDs to width/height). The `ntsc()` / `pal()` / `secam()` presets now fill the §5.0 known-tokens-table broadcast geometry (858×525 / 720×480 active for NTSC, 864×625 / 720×576 for PAL+SECAM) so the `dwHTotalInT` / `dwVTotalInLines` totals — which count the whole line/frame *including* blanking — differ from the active frame, matching how real OpenDML writers stamp the header. Pairs with the round-365 demuxer typed accessors for full demux↔mux symmetry (round 365)
- typed OpenDML 2.0 `vprp` signal-shape scalar accessors per the spec's *"Video Properties Header (vprp)"* §5.0 layout — `vprp_vertical_refresh_rate` (`dwVerticalRefreshRate`), `vprp_h_total_in_t` (`dwHTotalInT`), `vprp_v_total_in_lines` (`dwVTotalInLines`), `vprp_frame_width_in_pixels` (`dwFrameWidthInPixels`), `vprp_frame_height_in_lines` (`dwFrameHeightInLines`), each `-> Option<u32>` mapping a `0` field to `None` (writer-skipped sentinel; the matching `avi:vprp.<n>.*` metadata key is omitted there too) and `None` for streams with no `vprp`; plus `vprp_signal_shape(stream) -> Option<VprpSignalShape>` bundling all five DWORDs verbatim (zeros surfaced so a partially-specified header stays observable). These DWORDs were already emitted as `avi:vprp.<n>.*` metadata keys but had no typed surface, completing the `vprp` typed-accessor series alongside `vprp_video_format` / `vprp_video_standard` (round 336) and `vprp_frame_aspect_ratio` (round 104) (round 365)
- non-conformant reserved-field diagnostics on the OpenDML 2.0 `AVIMETAINDEX` headers per the spec's *"Index Structures"* layout: `super_index_reserved(stream) -> Option<[u32; 3]>` surfaces the AVISUPERINDEX `dwReserved[3]` array (*"0 if unused"*) only when at least one DWORD is non-zero, plus the `avi:indx.<n>.reserved` comma-joined `0x`-hex metadata key; `std_index_reserved(stream) -> Vec<(usize, u32)>` surfaces each `ix##` AVISTDINDEX `dwReserved3` (*"must be 0"*) as a `(segment, value)` pair when non-zero, in file order — both mirroring the round-330 `avih_reserved` "surface only the non-conformant case" shape (round 361)
- typed OpenDML 2.0 `AVIMETAINDEX` `bIndexType` decoder (`AviIndexType`: `OfIndexes` / `OfChunks` / `IsData` + `Other(u8)`, with `from_raw` / `to_raw` / `label` const methods) per the spec's *"Index Structures"* `// bIndexType codes` block: `super_index_index_type(stream) -> Option<AviIndexType>` (reports the declared type even for a present-but-mislabelled entry-less `indx`, `None` for no super-index) + `std_index_index_types(stream) -> Vec<AviIndexType>` (per-`ix##` segment, file order) + divergence-only `avi:indx.<n>.index_type` metadata key (emitted with the decoded label only when the super-index `bIndexType` ≠ the canonical `AVI_INDEX_OF_INDEXES`); the byte was previously read only to validate-and-discard (round 361)
- baseline DIB color table (`RGBQUAD bmiColors[]`) parsing for indexed-colour video streams (`biBitCount` 1/4/8) per the RIFF MCI reference §"Bitmap Color Table": `stream_palette(stream) -> Option<&[RgbQuad]>` (blue/green/red/reserved; `biClrUsed` entries, or `1 << biBitCount` when 0) + `avi:vids.<n>.palette_entries` count metadata key; absent for truecolour / `BI_BITFIELDS` / non-indexed streams (round 355)
- per-frame effective-palette resolution composing the baseline color table with `xxpc` `AVIPALCHANGE` deltas: `effective_palette_after_changes(stream, n)` (count-based, works without `idx1`), `effective_palette_at(stream, packet_seq)` (packet-sequence-based, `idx1`-positioned), and `palette_change_packet_positions(stream) -> &[u64]` (per-delta preceding-data-packet count) (round 355)
