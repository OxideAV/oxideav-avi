...
| File-global `avih.dwStreams` declared stream count (AVI 1.0 §"AVIMAINHEADER") | yes (`avih_declared_stream_count() -> Option<u32>` typed raw-DWORD accessor returning the verbatim 32-bit value at byte offset 24 of the 56-byte AVIMAINHEADER body + `declared_vs_actual_stream_count_mismatch() -> Option<(u32, u32)>` informational cross-check returning `(declared, actual)` whenever the non-zero declared count disagrees with the number of `strl` LISTs the demuxer actually walked in `hdrl`; per AVI 1.0 §"AVIMAINHEADER" Appendix A line 201: *"Number of streams in the file. For example, a file with audio and video has two streams."*; the writer-skips-it `0` sentinel parses as `None` so a degenerate / unspecified count reads the same as an absent one, mirroring the round-275/268/260/256/249/247/229/222/217/210/203/182/176/153/119/115 "default == absent" convention; round-292 adds the two typed surfaces on top of the pre-existing `avi:streams` metadata key (emitted verbatim, omitted only for the `0` sentinel); the cross-check is in the same informational family as `super_index_duration_violations` / `cbr_audio_block_alignment_violations` — it never fails `open()` and the demuxer always trusts the streams it physically parsed (`dwStreams` is advisory), but the `(declared, actual)` pair lets a downstream repair tool detect a truncated capture crash dump (header stamped up-front for N streams, file cut off before all N `strl` LISTs were written) or a hand-edited header) | yes (auto-derived `tracks.len()` stamp at `write_header`, so a round-trip through this crate's own writer always agrees with `Demuxer::streams().len()`; no override builder) |
| File-global `avih.dwWidth` × `dwHeight` movie rectangle (AVI 1.0 §"AVIMAINHEADER") | yes (`avih_movie_rect() -> Option<(u32, u32)>` typed accessor returning the `(width, height)` pair from byte offsets 32 + 36 of the 56-byte AVIMAINHEADER body; per AVI 1.0 §"AVIMAINHEADER" Appendix A lines 203/204: *"Width of the AVI file in pixels."* / *"Height of the AVI file in pixels."*; this is the file-global composition rectangle the per-stream `strh.rcFrame` destination rectangle (round-115) is expressed relative to — per the spec's `rcFrame` row the destination rectangle's upper-left corner is *"relative to the upper-left corner of the movie rectangle specified by the dwWidth and dwHeight members of the AVI main header structure"*, so a reader laying out multiple video/text streams onto a composite surface needs this pair to position each `rcFrame`; logically distinct from any single video stream's coded `BITMAPINFOHEADER.biWidth`/`biHeight` (equal for a single-video file, the union rectangle for multi-video) and surfaced verbatim with no cross-validation against any stream's coded size; a `0` in *either* dimension collapses the whole pair to `None` — a movie rectangle is only meaningful when both are non-zero, matching the round-249 `stream_timebase` "zero in either DWORD ⇒ None" shape; round-275 adds the typed accessor's "default == absent" mapping on top of the pre-existing `avi:width` / `avi:height` metadata keys, which stay emitted verbatim including `0`) | yes (auto-derived from the first video stream's coded `(width, height)` at `write_header` — `0` for audio-only files; no override builder, the file-global rectangle tracks the leading video stream's frame size; round-trips verbatim through the round-275 demuxer surface) |
...
| `LIST rec ` packet grouping inside `movi` + idx1 `rec ` LIST entries (AVI 1.0 §"AVI Index Entries" / Appendix C `AVIIF_LIST`) | yes (cluster walk during `next_packet` + round-285 typed `idx1_rec_list_entries() -> &[Idx1RecEntry]` / `idx1_rec_list_count()` accessors + `avi:idx1.rec_lists = "<N>"` metadata key — per the spec idx1 *"consists of an AVIOLDINDEX structure with entries for each data chunk, including 'rec ' chunks"*, flagged `AVIIF_LIST` (*"The chunk is a 'rec ' list."*); `rec ` entries carry no stream index so they never enter the per-stream seek / flags / side-band surfaces, and each entry's `offset` resolves file-absolute via the same movi-relative vs file-absolute probe as the seek table; round-285 also hardens that probe to skip non-per-stream entries — a leading `rec ` entry, whose target bytes are the `LIST` FourCC rather than the recorded ckid, previously could mis-anchor the offset-base detection of a file-absolute idx1; key/count omitted when zero so absence stays observable) | yes (packet-cap or byte-budget clustering; round-285 stamps one idx1 entry per cluster — ckid `rec `, `AVIIF_LIST`, `dwOffset` at the cluster's `LIST` header in the shared movi-relative base, `dwSize` back-patched to the LIST size-field value at cluster close — pushed at cluster open so idx1 stays in file order ahead of the grouped packets' entries; primary segment only since idx1 never spans `RIFF AVIX` continuations, and the `synthesise_idx1_from_ix` rebuild path indexes per-packet `ix##` records only, so it carries no `rec ` entries by design) |
