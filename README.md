...
| OpenDML 2.0 `LIST odml dmlh` extended header (OpenDML 2.0 §5.0 "Extended AVI Header" / "Total Frames") | yes (`dmlh_total_frames() -> Option<u64>` accessor returning the raw 32-bit value widened to `u64` from the 4-byte `ODMLExtendedAVIHeader.dwTotalFrames` DWORD + `avi:total_frames_all_segments` metadata key; `None` when no `LIST odml dmlh` was present, `Some(0)` for an explicit zero stamp — OpenDML 2.0 §5.0 does not define a "no length declared" sentinel for `dmlh.dwTotalFrames`, so `0` literally means zero frames and the typed accessor preserves that distinction from "no chunk"; logically distinct from `avih.dwTotalFrames` which §5.0 "Main AVI Header" pins to *"the size (number of frames) within the first RIFF 'AVI' chunk"* — `dmlh.dwTotalFrames` is the cross-segment count, `avih.dwTotalFrames` is the primary-RIFF count) | yes (auto-derives the cross-segment `total_video_frames` by default and back-patches it into the 4-byte `dmlh` body at `write_trailer` time; opt-in `AviMuxOptions::with_dmlh_total_frames(n)` override stamps `n` verbatim into the same DWORD instead, replacing the auto-derived value — last builder call wins; the override only changes the 4-byte stamp inside `LIST odml dmlh` and does NOT touch `avih.dwTotalFrames` (so file-level `Demuxer::duration_micros` keeps tracking the auto-derived primary-RIFF count), nor any downstream `idx1` / `ix##` / per-stream `strh.dwLength` derivation, so a caller that stamps a value incompatible with their actual cross-segment chunk count is creating an internally-inconsistent file on purpose; only meaningful for `AviKind::OpenDml` and ignored for `AviKind::Avi10`) |
